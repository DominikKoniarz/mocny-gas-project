...
export const releasesStore = {
    ...
    getLatestEnabled: (platform?: Platform): Release | undefined => {
        const enabled = releases
            ...
            .filter(
                (r) =>
                    !platform ||
                    (platform === "mac" ? r.macFile : r.windowsFile),
            )
    ...
    getStats: () => {
        ...
        const totalDownloads = releases.reduce((sum, r) => {
            return (
                sum +
                (r.macFile?.downloadCount || 0) +
                (r.windowsFile?.downloadCount || 0)
            );
        }, 0);
        const macDownloads = releases.reduce(
            (sum, r) => sum + (r.macFile?.downloadCount || 0),
            0,
        );
        const windowsDownloads = releases.reduce(
            (sum, r) => sum + (r.windowsFile?.downloadCount || 0),
            0,
        );
...
export const logsStore = {
    getAll: (filters?: LogFilters): UpdateLog[] => {
        ...
        if (filters?.version) {
            result = result.filter(
                (l) =>
                    l.toVersion === filters.version ||
                    l.fromVersion === filters.version,
            );
        }
    ...
    getStats: () => {
        ...
        const inProgress = logs.filter(
            (l) => l.status === "started" || l.status === "downloaded",
        ).length;
